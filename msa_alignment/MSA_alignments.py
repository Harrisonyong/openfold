...
import msa_alignment.tools.mmcif_parsing as mmcif_parsing
from msa_alignment.tools.data_pipeline import AlignmentRunner
from msa_alignment.tools.parsers import parse_fasta
...
def parse_and_align(files, alignment_runner, args):
    for f in files:
        ...
        print(f"=====Node{NODE_ID}:{threading.current_thread().name}_{file_id} is searching=====")
        start_time = time.time()
        run_seq_group_alignments(seq_group_tuples, alignment_runner, args)
        end_time = time.time()
        print(f"=====Node{NODE_ID}:{threading.current_thread().name}_{file_id} is end searching,using {round((end_time - start_time) / 60, 2)} minutes=====")
...
def main(args):
    ...
    for i, task_args in enumerate(task_arglist):
        print(f"======Node{NODE_ID}:Started thread {threading.current_thread().name}...{task_args}=====")
